...
class CasualGraph(nn.Module):
    ...
    def forward(self, node_embeddings, target_martrix,hypergraph_matrix,num_layers=3):
        ...
        # result=self.attetion(result) ## 第一个带dropout，第二个去掉了
        return result
class CasualGraph2(nn.Module):
    def __init__(self, node_dim, hidden_dim, output_dim, num_heads=1,num_layers=4,adj_matrix=None):
        super().__init__()
        self.node_dim = node_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
    ...
    def forward(self, node_embeddings,hypergraph_matrix,num_layers=3):
        last_embedding=node_embeddings
        result=[]
        H = hypergraph_matrix  # [N, E]
        N, E = H.shape
        for i in range(num_layers):
            target_embedding=torch.matmul(self.adj_matrix, last_embedding)#[4880,150]
            source_embedding=torch.matmul(self.adj_matrix.T, target_embedding)
            last_embedding=source_embedding
            last_embedding=last_embedding+node_embeddings
            last_embedding=self.norm(last_embedding)
            last_embedding=self.dropout(last_embedding)
