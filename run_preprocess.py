...
from preprocess.auxiliary import generate_code_code_adjacent, generate_neighbors, normalize_adj, divide_middle, generate_code_levels,generate_code_code_adjacent_level
...
if __name__ == '__main__':
    ...
    print('encoding code ...')
    admission_codes_encoded, code_map = encode_code(patient_admission, admission_codes) #将icd9code转化为数字形式
    code_num = len(code_map)
    print('There are %d codes' % code_num)
    code_levels = generate_code_levels(data_path, code_map) #分类
    pickle.dump({
    ...
    icd_adj=generate_code_code_adjacent_level(code_level_matrix=code_levels, code_num=code_num)
    ...
    pickle.dump(icd_adj, open(os.path.join(standard_path, 'auxiliary.pkl'), 'wb'))